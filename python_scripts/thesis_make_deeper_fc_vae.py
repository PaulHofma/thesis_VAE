...
class VAE(nn.Module):
    def __init__(self, z_dim=Z_DIM, h_dim=H_DIM, use_cuda=USE_CUDA, encoder=Encoder, decoder=Decoder):
        ...
        self.z_dim = z_dim
        self.h_dim = h_dim
        self.encoder = nn.DataParallel(encoder(z_dim, h_dim))
        self.decoder = nn.DataParallel(decoder(z_dim, h_dim))
        if use_cuda:
            self.cuda()
...
def main_1(dataset, train_n, test_n, z_dim=Z_DIM, h_dim=H_DIM, use_cuda=USE_CUDA, 
...
def big_plotter(used_vae, x=4, y=8, title="DEEPER FC MODEL RECONSTRUCTION SHEET", 
...
def small_plot(out_file, results, num_epochs, test_freq):
    ...
    axes.legend()
    fig.savefig(out_file/'elbo_plots_deeper_fc.png')
    axes.set_yscale('log')
    fig.savefig(out_file/'elbo_plots_deeper_fc_log.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hdim', default=H_DIM, type=int, help='(smallest) dimension of hidden space to use.')
    ...
    stringystring = "z="+str(Z_DIM)+"_h="+str(H_DIM)+"_N_Epochs="+str(NUM_EPOCHS)
    ...
    results1 = main_1(GD, train_n, test_n, z_dim=Z_DIM, h_dim=H_DIM, use_cuda=USE_CUDA, 
           encoder=Encoder, decoder=Decoder, base_lr=BASE_LR, num_epochs=NUM_EPOCHS,
           batch_size=BATCH_SIZE, **KWARGS) 
    ...
    stringystring = "ph-NOUPLOAD-T-vae_deeper_fc-decoder_z="+str(Z_DIM)+"_h="+str(H_DIM)+"_N_Epochs="+str(NUM_EPOCHS)+".pt"
    torch.save(my_decoder.state_dict(), str(output_path / stringystring))
    stringystring = "ph-NOUPLOAD-T-vae_deeper_fc-encoder_z="+str(Z_DIM)+"_h="+str(H_DIM)+"_N_Epochs="+str(NUM_EPOCHS)+".pt"
    torch.save(my_encoder.state_dict(), str(output_path / stringystring))

    # save elbo prog
    stringystring1 = "ph-NOUPLOAD-T-vae_deeper_fc-train_ELBO"+"_z="+str(Z_DIM)+"_h="+str(H_DIM)+"_N_Epochs="+str(NUM_EPOCHS)+".npy"
    stringystring2 = "ph-NOUPLOAD-T-vae_deeper_fc-test_ELBO"+"_z="+str(Z_DIM)+"_h="+str(H_DIM)+"_N_Epochs="+str(NUM_EPOCHS)+".npy"
    np.save(str(output_path / stringystring1), np.array(results1[3]))
    np.save(str(output_path / stringystring2), np.array(results1[4]))
    
    #few visuals
    big_plotter(used_vae=results1[0], out_path=output_path, offset=train_n)
    small_plot(output_path, results1, NUM_EPOCHS, TEST_FREQUENCY)
