...
def main_1(dataset, train_n, test_n, z_dim=Z_DIM, f_dim=F_DIM, use_cuda=USE_CUDA, 
           ...
           batch_size=BATCH_SIZE, beta=BETA, **kwargs):
...
#     optimizer = pyro.optim.StepLR({'optimizer': optimizer, 'optim_args': {'lr': BASE_LR}, 'step_size':LR_STEP, 'gamma': LR_GAMMA})
    ...
    svi = SVI(vae.model, vae.guide, optimizer, loss=Beta_ELBO(beta))
...
if __name__ == '__main__':
    ...
    stringystring = f"beta_conv_z={Z_DIM}_f={F_DIM}_beta={BETA}_N_Epochs={NUM_EPOCHS}"
    ...
    stringystring = f"ph-NOUPLOAD-T-vae_beta_conv-decoder_z={Z_DIM}_f={F_DIM}_beta={BETA}_N_Epochs={NUM_EPOCHS}.pt"
    torch.save(my_decoder.state_dict(), str(output_path / stringystring))
    stringystring = f"ph-NOUPLOAD-T-vae_beta_conv-encoder_z={Z_DIM}_f={F_DIM}_beta={BETA}_N_Epochs={NUM_EPOCHS}.pt"
    torch.save(my_encoder.state_dict(), str(output_path / stringystring))
    
    # save elbo prog
    stringystring = f"ph-NOUPLOAD-T-vae_beta_conv-train_ELBO_z={Z_DIM}_f={F_DIM}_beta={BETA}_N_Epochs={NUM_EPOCHS}.npy"
    np.save(str(output_path / stringystring), np.array(results1[3]))
    stringystring = f"ph-NOUPLOAD-T-vae_beta_conv-test_ELBO_z={Z_DIM}_f={F_DIM}_beta={BETA}_N_Epochs={NUM_EPOCHS}.npy"
    np.save(str(output_path / stringystring), np.array(results1[4]))
    
    #few visuals
    big_plotter(used_vae=results1[0], out_path=output_path, offset=train_n, title=f"BETA CONV MODEL (BETA={BETA}) RECONSTRUCTION SHEET")
    small_plot(output_path, results1, NUM_EPOCHS, TEST_FREQUENCY, title=f"elbo_plots_beta_conv_b={BETA}")
