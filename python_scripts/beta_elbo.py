...
class Beta_ELBO(Trace_ELBO):
    """
    Beta-ELBO for implementing a Beta-VAE; based on Trace_Meanfield_ELBO
    since that is the only ELBO estimator that uses KLdiv to calculate
    the ELBO. 
    ----
    ORIGINAL:
    ...
    def __init__(self, beta=1., *args, **kwargs):
        print("initializing beta elbo")
        super().__init__(*args, **kwargs)
        self.beta = beta
    
    def _get_trace(self, model, guide, *args, **kwargs):
        model_trace, guide_trace = super(Beta_ELBO, self)._get_trace(
            ...
            if model_site["type"] == "sample":
                if model_site["is_observed"]:
                    ...
                    try:
                        ...
                        elbo_particle = elbo_particle - self.beta*kl_qp.sum() ##beta!
                    except NotImplementedError:
                        entropy_term = guide_site["score_parts"].entropy_term
                        elbo_particle = elbo_particle + model_site["log_prob_sum"] - self.beta*entropy_term.sum() ##pretty sure this is where it goes otherwise.
