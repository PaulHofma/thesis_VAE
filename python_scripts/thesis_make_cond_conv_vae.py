...
from ...src.setup_cosmos_data_loaders_optimized import COSMOSDataset, NoiseMeanSubtract, Normalize, Denoise, ToTensor
# from beta_elbo import Beta_ELBO
...
F_DIM = 32
C_DIM = 5
# BETA = 10 #[5,10,20,50,100,500,1000]
BATCH_SIZE = 12
CHUNK_SIZE = 512
...
class Encoder(nn.Module):
    def __init__(self, z_dim, f_dim, c_dim):
        super(Encoder, self).__init__()
        self.z_dim = z_dim
        self.f_dim = f_dim
        self.c_dim = c_dim
        self.fc1 = nn.Linear(IMG_DIMS[0]*IMG_DIMS[1]+c_dim, IMG_DIMS[0]*IMG_DIMS[1])
#         self.bn1 = nn.BatchNorm2d(1)
        self.layers = nn.Sequential(
            nn.Conv2d(1, self.f_dim, kernel_size=3, stride=1, padding=1),
            nn.ELU(),
            nn.BatchNorm2d(self.f_dim),
...
#         print(x.shape)
#         print()
        x = torch.cat((x,cond_as_arr(c)), dim=-1)
#         print(x.shape)
        x = self.fc1(x)
...
class Decoder(nn.Module):
    def __init__(self, z_dim, f_dim, c_dim):
        ...
        self.f_dim = f_dim
        self.c_dim = c_dim
        self.fc1 = nn.Linear(self.z_dim+self.c_dim, 64)
#         self.bn1 = nn.BatchNorm2d(1)
        self.layers = nn.Sequential(
...
#         print(z.shape)
        z = torch.cat((z,cond_as_arr(c)), dim=-1)
        z = self.fc1(z)
        z = z.reshape(-1, 1, 8, 8)
#         z = self.bn1(z)
        z = self.layers(z)
...
def cond_as_arr(info_in, key_list=['noise_variance', 'r_e', 'index', 'q', 'z']):
    info_out = torch.transpose(torch.stack([info_in[key] for key in key_list]), 0, 1)
    return info_out
...
class VAE(nn.Module):
    def __init__(self, z_dim=Z_DIM, f_dim=F_DIM, use_cuda=USE_CUDA, encoder=Encoder, decoder=Decoder):
        ...
        self.z_dim = z_dim
        self.f_dim = f_dim
        self.encoder = nn.DataParallel(encoder(z_dim, f_dim, c_dim=C_DIM))
        self.decoder = nn.DataParallel(decoder(z_dim, f_dim, c_dim=C_DIM))
        if use_cuda:
            self.cuda()
    ...
    def model(self, x, c):
        ...
        with pyro.plate("data", x.shape[0]):
            ...
            img_loc = self.decoder.forward(z, c)
            img_scale = 0.02 #noise = 1/50
...
#             pyro.sample("obs", dist.Bernoulli(img_loc).to_event(1), obs=x.reshape(-1, IMG_DIMS[0]*IMG_DIMS[1]))
    ...
    def reconstruct_img(self, x, c):
        # encode image x
        z_loc, z_scale = self.encoder(x, c)
        # sample in latent space
        z = dist.Normal(z_loc, z_scale).sample()
        # decode the image (note we don't sample in image space)
        img_loc = self.decoder(z, c)
...
def evaluate(svi, dataset, train_n, test_n, use_cuda=USE_CUDA, batch_size=BATCH_SIZE):
    ...
    for i in np.arange(train_n, train_n+test_n, step=batch_size):
        ...
        if use_cuda:
            img = img.cuda()
        test_loss += svi.evaluate_loss(img, info)
        del(img)
        del(info)
...
def big_plotter(used_vae, x=4, y=8, title="BASIC FC MODEL RECONSTRUCTION SHEET", 
    ...
    img, info = GD[offset:x*y+offset]
    if use_cuda:
        img = img.cuda()
#                 info = info.cuda()
    rec = used_vae.reconstruct_img(img, info)
    img = np.array(img.detach().cpu().reshape(-1, 64,64))
    rec = np.array(rec.detach().cpu().reshape(-1, 64,64))

    for i in range(x):
        for j in range(y):
            max_val = np.amax([np.amax(img[i*y + j+offset]), np.amax(rec[i*y + j+offset])])
            min_val = np.amin([np.amin(img[i*y + j+offset]), np.amin(rec[i*y + j+offset])])
...
def small_plot(out_file, results, num_epochs, test_freq, title='elbo_plots_basic_conv'):
...
#     parser.add_argument('--beta', default=BETA, type=int, help='For Beta-VAE; amplification factor to use for KLD (=BETA)')
...
#     BETA = args.beta
    ...
    stringystring = f"cond_conv_z={Z_DIM}_f={F_DIM}_N_Epochs={NUM_EPOCHS}"
    ...
    stringystring = f"ph-NOUPLOAD-T-vae_cond_conv-decoder_z={Z_DIM}_f={F_DIM}_N_Epochs={NUM_EPOCHS}.pt"
    torch.save(my_decoder.state_dict(), str(output_path / stringystring))
    stringystring = f"ph-NOUPLOAD-T-vae_cond_conv-encoder_z={Z_DIM}_f={F_DIM}_N_Epochs={NUM_EPOCHS}.pt"
    torch.save(my_encoder.state_dict(), str(output_path / stringystring))
    
    # save elbo prog
    stringystring = f"ph-NOUPLOAD-T-vae_cond_conv-train_ELBO_z={Z_DIM}_f={F_DIM}_N_Epochs={NUM_EPOCHS}.npy"
    np.save(str(output_path / stringystring), np.array(results1[3]))
    stringystring = f"ph-NOUPLOAD-T-vae_cond_conv-test_ELBO_z={Z_DIM}_f={F_DIM}_N_Epochs={NUM_EPOCHS}.npy"
    np.save(str(output_path / stringystring), np.array(results1[4]))
    
    #few visuals
    big_plotter(used_vae=results1[0], out_path=output_path, offset=train_n, title=f"COND CONV MODEL RECONSTRUCTION SHEET")
    small_plot(output_path, results1, NUM_EPOCHS, TEST_FREQUENCY, title=f"elbo_plots_cond_conv")
