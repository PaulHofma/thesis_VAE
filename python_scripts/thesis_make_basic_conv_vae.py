...
class Encoder(nn.Module):
    def __init__(self, z_dim, f_dim):
        ...
        self.fc21 = nn.Linear(1024*self.f_dim, self.z_dim)
        self.fc22 = nn.Linear(1024*self.f_dim, self.z_dim)

    def forward(self, x):
        x = x.reshape(-1, 1, IMG_DIMS[0], IMG_DIMS[1])
        x = self.layers(x)
        x = x.reshape(-1, 1024*self.f_dim)
        z_loc = self.fc21(x)
...
#         self.fc1 = nn.Linear(self.z_dim, 64)
        self.layers = nn.Sequential(
            nn.ConvTranspose2d(1, self.f_dim, kernel_size=3, stride=2, padding=1),
            nn.ELU(),
        )
        self.fc21 = nn.Linear(15*15*self.f_dim, 4096)
...
#         z = self.fc1(z)
        z = z.reshape(-1, 1, 8, 8)
        z = self.layers(z)
        z = z.reshape(-1, 15*15*self.f_dim)
        img_loc = self.fc21(z)
...
def small_plot(out_file, results, num_epochs, test_freq, title='elbo_plots_basic_conv.png'):
...
if __name__ == '__main__':
    ...
    stringystring = "basic_conv_z="+str(Z_DIM)+"_f="+str(F_DIM)+"_N_Epochs="+str(NUM_EPOCHS)
    ...
    stringystring = "ph-NOUPLOAD-T-vae_basic_conv-decoder_z="+str(Z_DIM)+"_f="+str(F_DIM)+"_N_Epochs="+str(NUM_EPOCHS)+".pt"
    torch.save(my_decoder.state_dict(), str(output_path / stringystring))
    stringystring = "ph-NOUPLOAD-T-vae_basic_conv-encoder_z="+str(Z_DIM)+"_f="+str(F_DIM)+"_N_Epochs="+str(NUM_EPOCHS)+".pt"
    torch.save(my_encoder.state_dict(), str(output_path / stringystring))
    
    # save elbo prog
    stringystring1 = "ph-NOUPLOAD-T-vae_basic_conv-train_ELBO"+"_z="+str(Z_DIM)+"_f="+str(F_DIM)+"_N_Epochs="+str(NUM_EPOCHS)+".npy"
    stringystring2 = "ph-NOUPLOAD-T-vae_basic_conv-test_ELBO"+"_z="+str(Z_DIM)+"_f="+str(F_DIM)+"_N_Epochs="+str(NUM_EPOCHS)+".npy"
    np.save(str(output_path / stringystring1), np.array(results1[3]))
    np.save(str(output_path / stringystring2), np.array(results1[4]))
    
    #few visuals
    big_plotter(used_vae=results1[0], out_path=output_path, offset=train_n, title="BASIC CONV MODEL RECONSTRUCTION SHEET")
    small_plot(output_path, results1, NUM_EPOCHS, TEST_FREQUENCY, title='elbo_plots_basic_conv.png')
